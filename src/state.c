...
void state_update(level *lvl, state *sta){
    ...
    for(int i=0;i<sta->n_enemies;i++){
        float distance = sqrt(((sta->enemies[i].ent.x - sta->pla.ent.x)*(sta->enemies[i].ent.x - sta->pla.ent.x))+((sta->enemies[i].ent.y - sta->pla.ent.y)*(sta->enemies[i].ent.y - sta->pla.ent.y)));
        if(sta->enemies[i].kind == MINION)
        {
            ...
            {
//                printf("vx = %f vy = %f \n", (sta->pla.ent.x - sta->enemies[i].ent.x)/distance, (sta->pla.ent.y - sta->enemies[i].ent.y)/distance);
        ...
        else
        {
            ...
            {
                sta->enemies[i].ent.vx = (sta->pla.ent.x - sta->enemies[i].ent.x)*4/distance;
                sta->enemies[i].ent.vy = (sta->pla.ent.y - sta->enemies[i].ent.y)*4/distance;
            }
            else if(distance >= 160)
            {
                sta->enemies[i].ent.vx = 0;
